/**
 * \file
 *      Observable "on-change" luminosity resource based on example code
...
res_get_handler(void *request, void *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
   ...
   */
	...
	PRINTF("Get handler for luminosity sensor called.\n");  // For debugging.
  ...
  if(accept == -1 || accept == REST.type.TEXT_PLAIN) {
	...
	snprintf((char *)buffer, REST_MAX_CHUNK_SIZE, "{'luminosity':%d [Lux]}", lumi);
...
res_periodic_handler()
{
	PRINTF("Periodic handler for luminosity sensor called.\n");  // For debugging.
  ...
  if((abs(lumi - lumi_old) >= CHANGE && interval_counter >= INTERVAL_MIN) || 
     interval_counter >= INTERVAL_MAX) {
     interval_counter = 0;
     lumi_old = lumi;
    /* Notify the registered observers which will trigger the res_get_handler to create the response. */
    REST.notify_subscribers(&res_derfnode_isl29020);
  }
}
#endif /* DE_RF_NODE */
